 ...
 *
 * Для child роли на desktop: PhoneFrame (390×844 с тёмным border) + ChildTopBar.
 */
export const AppShell: React.FC<AppShellProps> = ({ children, showNav = true }) => {
  ...
  // === CHILD: PhoneFrame на desktop, plain на mobile ===
  if (isChild) {
    return (
      <PhoneFrame>
        <div className="w-full h-full bg-bg flex flex-col relative">
          <ChildTopBar />
          <main className={`flex-1 overflow-y-auto ${showNav ? 'pb-28' : 'pb-8'}`}>
            {children}
          </main>
  ...
  // === Parent / Tutor / Specialist: standard max-width panel ===
