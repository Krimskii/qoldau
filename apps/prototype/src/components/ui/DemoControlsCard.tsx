    ...
    <QoldauCard variant="elevated" padding="md">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-black text-ink">{t('demoControls.title')}</h3>
          <p className="text-xs text-muted leading-relaxed mt-0.5">
            {t('demoControls.description')}
          </p>
          <div className="mt-3 flex items-center gap-2 flex-wrap">
            ...
            >
              ...
              {t('demoControls.reset')}
            </button>
            {lastReset && resetCount !== null && (
              <span className="text-[10px] text-muted flex items-center gap-1">
                <AppIcon component={Check} size={12} colorClass="text-teal" />
                {t('demoControls.resetDone', { count: resetCount, time: formatTime(lastReset) })}
              </span>
            )}
          </div>
        </div>
